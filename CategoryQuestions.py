...
class QuizByCategory:
    ...
    def save_score(self, name, score):
        if os.path.exists('category-score.json') and os.path.getsize('category-score.json') > 0:
            with open('category-score.json', 'r') as file:
                data = json.load(file)
        else:
            data = []
        ...
        with open('category-score.json', 'w') as file:
            json.dump(data, file, indent=4)

    def play_quiz(self, selected_category, player_name):
